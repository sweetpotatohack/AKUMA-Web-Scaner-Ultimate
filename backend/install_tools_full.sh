...
apt install -y --no-install-recommends \
    ...
    libffi-dev zlib1g-dev python3-dev whatweb || { 
...
if ! command -v pipx &>/dev/null; then
    ...
    if ! command -v wpscan &>/dev/null; then
        ...
        if gem install wpscan; then
            ...
            if gem install wpscan --user-install; then
                ...
                echo "⚠ Сканирование WordPress будет пропущено"
            fi
        fi
    fi
    ...
    if command -v wpscan &>/dev/null; then
        wpscan_version=$(wpscan --version 2>/dev/null | head -n 1 | awk '{print $2}' || echo "unknown")
...
if ! command -v bbot &>/dev/null; then
    if command -v pipx &>/dev/null; then
        ...
        pip3 uninstall -y beautifulsoup4 bs4 2>/dev/null || true
        # Устанавливаем совместимую версию bs4
        pip3 install beautifulsoup4==4.12.0 2>/dev/null || true
        
        # pipx install с нужным флагом
        if pipx install bbot 2>/dev/null; then
            bbot_path=$(pipx list --short 2>/dev/null | grep bbot | awk '{print $3}')
            if [ -n "$bbot_path" ]; then
                cp "$bbot_path/bin/bbot" /usr/local/bin/ 2>/dev/null || echo "⚠ Не удалось скопировать BBOT в /usr/local/bin"
            fi
        ...
        echo "⚠ pipx недоступен, пропускаем установку BBOT"
    fi
else
    ...
    if [ ! -f "/root/cloud_enum/cloud_enum.py" ]; then
        ...
        if python3 -m venv /opt/cloud_enum_venv && \
           ...
           pip3 install -r /root/cloud_enum/requirements.txt; then
            deactivate
    ...
    if [ ! -f "/root/check_bitrix/test_bitrix.py" ]; then
        ...
        if git clone https://github.com/k1rurk/check_bitrix.git /root/check_bitrix && \
           python3 -m pip install -r /root/check_bitrix/requirements.txt; then
    ...
    # Установка nmap-did-what
    if [ ! -d "/root/nmap-did-what" ]; then
        echo "Установка nmap-did-what..."
        if git clone https://github.com/hackertarget/nmap-did-what.git /root/nmap-did-what; then
            echo "✅ nmap-did-what успешно установлен"
        else
            echo "⚠ Не удалось клонировать nmap-did-what"
        fi
    fi
